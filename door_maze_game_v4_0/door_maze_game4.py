...
IMAGE_DIR = Path(sys.path[0]) / "door_maze_images"
...
class MyForm(QtWidgets.QMainWindow):
    ...
    def clear(self):
        self.image_names = {
            "room": "door_maze_room3.png",
            "room2": "door_maze_room4.png",
            "west": "door_west2b.png",
            "east": "door_east2b.png",
            "north": "door_north2b.png",
            "south": "door_south2b.png",
            "ramp": "maze_ramp1.png",
    ...
    def getDir(self):
        currentDir = Path(sys.path[0])
        os.chdir(currentDir)
        return str(currentDir)

    def load(self):
        filename, ext1 = QtWidgets.QFileDialog.getOpenFileName(
            self, "Open Maze", self.currentDir, "*.mz"
        )
        if filename:
    ...
    def generate_px(self):
        try:
            self.room_px = QtGui.QPixmap(str(IMAGE_DIR / self.image_names["room"]))
            self.room2_px = QtGui.QPixmap(str(IMAGE_DIR / self.image_names["room2"]))
            self.mouse_px = QtGui.QPixmap(
                str(IMAGE_DIR / self.image_names["mouse_image"])
            )
            self.west_px = QtGui.QPixmap(str(IMAGE_DIR / self.image_names["west"]))
            self.east_px = QtGui.QPixmap(str(IMAGE_DIR / self.image_names["east"]))
            self.north_px = QtGui.QPixmap(str(IMAGE_DIR / self.image_names["north"]))
            self.south_px = QtGui.QPixmap(str(IMAGE_DIR / self.image_names["south"]))
            self.wclosed_px = QtGui.QPixmap(
                str(IMAGE_DIR / self.image_names["closed_west"])
            )
            self.eclosed_px = QtGui.QPixmap(
                str(IMAGE_DIR / self.image_names["closed_east"])
            )
            self.nclosed_px = QtGui.QPixmap(
                str(IMAGE_DIR / self.image_names["closed_north"])
            )
            self.sclosed_px = QtGui.QPixmap(
                str(IMAGE_DIR / self.image_names["closed_south"])
            )
            self.ramp_px = QtGui.QPixmap(str(IMAGE_DIR / self.image_names["ramp"]))
            self.win_px = QtGui.QPixmap(str(IMAGE_DIR / self.image_names["final_room"]))
        except WindowsError:
            self.room_px = None
            self.room2_px = None
            self.mouse_px = None
    ...
    def disp_cheats(self):
        ...
        if re.search("(mouse|mice)", chStr.lower()):
            ...
            message = "location:\n{0:d}\n{sym1}\n{alph1}\n".format(
                roomNum1, sym1=roomSym1, alph1=roomAlph1
            )
